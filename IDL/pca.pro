...
;
; CATEGORY:
;    Time Series Analysis, Optimal Detection Package v3.1.1
;
; CALLING SEQUENCE:
;    PCA, DATA
;
...
;        spatiotemporal data, N_POINT is the number of spatial elements while 
;        N_SAMPLE is the number of time elements.
;
; KEYWORD PARAMETERS:
...
;    EVECTOR:  An output matrix containing the empirical orthogonal functions 
;        (EOFs, = eigenvectors of the covariance matrix), of format 
;        N_POINT*N_PC.
;    NAN:  If set, the procedure will ignore missing values (tagged as NaN) in 
;        the input data.  The default is to include NaNs.
;    NO_ANOMALY:  If set, the means are not subtracted from each point in the 
;        input data.  The default is to subtract the mean (and thus estimate 
;        the covariance matrix).
;    NORMALISE_PC:  If set, then all principal components in PC are normalised 
;        to unit variance when weighted.  The default is to include the 
;        variance in the decomposition in the principal components in PC.
;    OUTDATA:  An output matrix containing the modified version of the input 
;        matrix Data used for the estimation (e.g. with means subtracted).
;    PC:  An output matrix containing the principle components, of format 
;        N_SAMPLE*N_PC.
;    SVD:  If set then the calculations are performed via the singular value 
;        decomposition of DATA using svdc.  The default is to perform 
;        eigen-reduction using either eigenql or la_eigenql.
;    WEIGHT_POINT:  A vector of length N_POINT containing weightings for the 
;        DATA point fields.
;    WEIGHT_SAMPLE:  A vector of length N_SAMPLE containing weightings for the 
;        DATA sample fields.
;    UNWEIGHT:  If set, the EOFs and PCs are unweighted before being returned.  
;        The default is the return of weighted EOFs and PCs.  Note this does 
;        not matter if WEIGHT_POINT and WEIGHT_SAMPLE are not set.
;
; OUTPUT:
...
;    eigenvalues and eigenvectors of the covariance matrix of the data.  The 
;    pcomp routine does this too, but does not incorporate weighting, and seems 
;    to be slower and more hungry for memory.
...
;        Monthly Weather Review, 110, 699-706.
;
; EXAMPLE:
...
;      n_time = 20
;      n_space = 16
;      data = fltarr( n_space, n_time )
;      for i_time = 0, n_time - 1 do begin
;        data[*,i_time] $
;          = cos( findgen( n_space ) / ( n_space - 1. ) * 2. * !pi ) $
;          * cos( 2. * i_time / ( n_time - 1. ) * 2. * !pi ) $
;          + 0.5 * sin( 3. * findgen( n_space ) / ( n_space - 1. ) * 2. * !pi) $
;	   * sin( 4. * i_time / ( n_time - 1. ) * 2. * !pi ) $
;          + 0.2 * randomn( 1, n_space )
;      endfor
;    Calculate the principal components.
;      pca, data, evalue=evalue, evector=evector, pc=pc
;
; MODIFICATION HISTORY:
;    Written by:  Daithi Stone (stoned@atm.ox.ac.uk), 2001-05-11.
...
;    Modified:  DAS, 2004-10-27 (added capability for complex input, very large 
;        input matrices; added OUTDATA keyword)
;    Modified:  DAS, 2005-08-05 (replaced SUM.PRO use with TOTAL)
;    Modified:  DAS, 2007-04-02 (fixed bug in implementation of TOTAL)
;    Modified:  DAS, 2007-10-31 (fixed bug in covariance calculation;  changed 
;        WEIGHT keyword to WEIGHT_POINT;  added WEIGHT_SAMPLE keyword;  cleaned 
;        code and documentation)
;    Modified:  DAS, 2011-10-14 (Edited documentation formatting;  removed use 
;        of real.pro;  added NORMALISE_PC option;  fixed bug of EVECTOR being 
;        returned as single when DOUBLE set;  inclusion in Optimal Detection 
;        Package category)
;    Modified:  DAS, 2012-01-23 (Added SVD option)
;-
...
PRO PCA, $
    ...
    UNWEIGHT=unweight_opt, NORMALISE_PC=normalise_pc_opt, $
...
; Option to use singular value decomposition in the calculations
svd_opt = keyword_set( svd_opt )
...
if keyword_set( correlation_opt ) then begin
  ...
  for i = i_point_0, n_point - 1 do begin
    if complex_flag eq 0 then begin
      ...
      temp_std = total( real_part( weight_sample * temp_std ), nan=nan_opt ) $
          / total( weight_sample * finite( temp_std ) )
    endelse
  endfor
  temp_std = 0
...
if n_sample lt n_point then begin
  ; For the SVD approach take the transpose of data_w and do not calculate the 
  ; covariance matrix
  if svd_opt eq 1 then begin
    data_w_svd = reform( transpose( data_w ), n_sample, n_point )
  ; For the eigen-reduction approach
  endif else begin
...
; Otherwise do things normally
  ...
  ; For the SVD approach, take the data as-is and do not calculate the 
  ; covariance matrix
  if svd_opt eq 1 then begin
    data_w_svd = reform( data_w, n_point, n_sample )
  ; For the eigen-reduction approach
  endif else begin
    ; Initialise the covariance matrix
...
; Do the eigen-reduction for a complex array
if complex_flag eq 0 then begin
  ; Using the SVD approach
  if svd_opt eq 1 then begin
    ; Do this whichever way is faster
    if n_point gt n_sample then begin
      svdc, data_w_svd, e_value, e_vector, pc, double=double_opt
    endif else begin
      svdc, data_w_svd, e_value, pc, e_vector, double=double_opt
    endelse
  ; Using the eigen-reduction approach
  endif else begin
    e_value = eigenql( cov, double=double_opt, eigenvectors=e_vector, $
        overwrite=1 )
  endelse
; Do the eigen-reduction for a real array
endif else begin
  ; Using the SVD approach
  if svd_opt eq 1 then begin
    ; Do this whichever way is faster
    if n_point gt n_sample then begin
      la_svd, data_w_svd, e_value, e_vector, pc, double=double_opt
    endif else begin
      la_svd, data_w_svd, e_value, pc, e_vector, double=double_opt
    endelse
  ; Using the eigen-reduction approach
  endif else begin
...
; Convert SVD output to the format expected for PCA
if svd_opt eq 1 then begin
  ; Clear memory
  data_w_svd = 0
  ; Convert singular values to eigenvalues
  e_value = ( e_value ^ 2. ) / n_sample
  ; Transpose eigenvector and PC arrays
  e_vector = transpose( e_vector )
  pc = transpose( pc )
  n_pc = n_elements( pc[0,*] )
  ; Arrange in order of decreasing singular value
  id = reverse( sort ( e_value ) )
  e_value = e_value[id]
  e_vector = e_vector[*,id]
  pc = pc[*,id]
endif

; Convert point eigenvectors back to sample eigenvectors if necessary (see 
; above, already done for SVD)
if ( svd_opt eq 0 ) and ( n_sample lt n_point ) then begin
  ; Normalise eigenvalues
  e_value = e_value * n_point / n_sample
  ; Convert back to sample eigenvectors
  temp = one * fltarr( n_point, n_sample )
  for i = i_sample_0, n_sample - 1 do begin
...
; Estimate the principal components (already done with SVD approach).
if svd_opt eq 0 then begin
...
; Normalise PCs if requested (i.e. set standard deviation to unity)
if keyword_set( normalise_pc_opt ) and ( svd_opt eq 0 ) then begin
  for i = 0, n_pc - 1 do pc[*,i] = pc[*,i] / sqrt( total( pc[*,i] ^ 2 ) )
endif
; Un-normalise the PCs for the SVD approach
if not( keyword_set( normalise_pc_opt ) ) and ( svd_opt eq 1 ) then begin
  for i = 0, n_pc - 1 do pc[*,i] = pc[*,i] * sqrt( e_value[i] )
endif
