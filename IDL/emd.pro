...
while check lt checkexitval do begin
  ...
  nextrema = n_elements( extrema2( x ) )
  ...
  while check eq 0 do begin
    ...
    ; Find local extrema for minimum and maximum envelopes
    ;temp = extrema2( x, minima=minima, maxima=maxima, /flat )
    temp = extrema2( x, minima=minima, maxima=maxima )
...
;      localmean = spline( meanpos, meanval, indgen( ndata ) )
      localmean = interpol(meanval, meanpos, datagen, /spline)
...
;      maxenv = interpol(  maxval, maxpos, datagen,/spline )
;      minenv = interpol( minval, minpos, datagen,/spline)

      maxspl = spl_init(maxpos,maxval)
      maxenv = spl_interp(maxpos,maxval,maxspl,datagen)

      minspl = spl_init(minpos,minval)
      minenv = spl_interp(minpos,minval,minspl,datagen)
    ...
    if zerocrossopt then begin
     ...
     ; nzeroes = zero_cross( x )
      nzeroes = n_elements(uniq(x GE 0))
  ...
  if var_type( imf ) eq 0 then begin
    ...
    imf = x
    print,'Extracted first IMF'
  endif else begin
    ; If this is a later IMF
    imf = [ [imf], [x] ]
    print,'Extracted another IMF'
  endelse
